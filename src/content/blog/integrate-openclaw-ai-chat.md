...
所以加了一层 **API Bridge**（Node.js Express 服务）：

```
浏览器 → Nginx → API Bridge (:3001) → OpenClaw (:18789)
```

Bridge 负责：注入 Kaito AI 分身人格、限流保护、格式化响应。前端只和 Bridge 通信，完全不感知后端的 OpenClaw。

## API Bridge 三个端点

| 端点 | 方法 | 功能 |
|------|------|------|
| `/api/chat` | POST | 自由聊天，注入 system prompt 定义人格 |
| `/api/article-qa` | POST | 基于文章内容的问答，自动注入文章全文做上下文 |
| `/api/health` | GET | 健康检查 |

### System Prompt 设计

定义 Kaito 的 AI 分身人格是关键一步。我给它设了三个维度的约束：
...
AI 调用是有成本的（即使是本地跑），不加限制容易被滥用。三级限流：
...
用 `express-rate-limit` 实现，超限返回 HTTP 429 + 友好提示。

## 前端组件

遵循 Astro 的"零 JS 默认"理念，全部用内联 `<script is:inline>` + IIFE 实现，不引入任何框架。

### 浮窗 AI 聊天

右下角 48px 紫蓝渐变圆形按钮，外层有脉冲光环动画，视觉上比返回顶部按钮更显眼。

点击弹出 380×500px 对话框，包含：
- **头部**：💬 Kaito AI · 在线 · 随时可聊
- **消息区**：用户消息靠右紫蓝渐变气泡，AI 回复靠左灰色气泡，颜色区分但不加机械标签
- **输入区**：支持 Enter 发送 / Shift+Enter 换行
- **打字指示器**：三个跳动圆点，等 AI 回复时不焦虑
...
点击预设问题会自动把问题 + 文章标题 + 文章内容（前 8000 字）发给 API Bridge，后者构造包含上下文的 prompt 再请求 OpenClaw。

## 部署

API Bridge 是一个独立的 Node.js 服务，部署在服务器 `/opt/api-bridge/`，通过 systemd 管理：
...
前端用相对路径 `/api/chat` 发请求，IP 或域名都能正常工作。
